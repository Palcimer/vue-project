    ...
    <v-tooltip top>
      <template v-slot:activator="{ on, attrs }">
        <v-btn
          ...
          :color="goodColor"
          @click.stop="likeUpdate(1)"
        >
        <!-- @click.stop : 이벤트 전파(클릭했을 때 아래쪽 컴포넌트로 클릭을 전파)를 막음 -->
  ...
  methods: {
    async likeUpdate(bg_flag) {
        ...
        if(bg_flag == this.likeFlag) {
            ...
            data = await this.$axios.delete(`/api/like/${this.table}/${this.item.wr_id}`);
        } else {
            // 변경되는 경우
            data = await this.$axios.post(`/api/like/${this.table}/${this.item.wr_id}`, { bg_flag });
        }
        if(data) {
